...
mkdir -p "$destination"
cp geist-app geistd "$destination/"
cp scripts/start-geist.sh "$destination/Start Geist.sh"
chmod 755 "$destination/Start Geist.sh" "$destination/geist-app" "$destination/geistd"
...
(cd "$destination" && if command -v sha256sum >/dev/null; then sha256sum geist-app geistd > SHA256SUMS; else shasum -a 256 geist-app geistd > SHA256SUMS; fi)
